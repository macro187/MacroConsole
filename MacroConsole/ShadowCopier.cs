...
namespace MacroConsole
{
    ...
    public static class ShadowCopier
    {
        ...
        const string ShadowDirName = "_shadow";
        ...
        /// <remarks>
        /// <para>
        /// Actually, move the calling program's running binaries to a <c>_shadow</c> subdirectory, maintaining unlocked
        /// copies in the original locations which can be replaced or removed while the program is still running.
        /// </para>
        /// <para>
        /// Prior to moving, all assemblies statically referenced by the calling program are preloaded so the running
        /// process binds to them, otherwise it may lazy load and bind after the copy is perfomed, defeating the
        /// purpose.
        /// </para>
        /// <para>
        /// Shortcomings:
        ...
        /// - Probably fails if more than one instance of the program is run at the same time, because both processes
        ///   would try to use the same shadow subdirectory
        /// </para>
        /// </remarks>
        ///
        ...
        static void PreloadReferencedAssemblies(Assembly assembly)
        {
            PreloadReferencedAssemblies(assembly, new HashSet<string>());
        }


        static void PreloadReferencedAssemblies(Assembly assembly, ISet<string> loaded)
        {
